...
//         Parlay memory p = bets[_betID];
//         bool check = true;
//         for(uint i=0;i<p.indexes.length;i++){
//             uint256 timestamp = matchCancellationTimestamp[p.index[i]];
...
//         MAX_BET = dai.allowance(treasury,address(this)).mul(5).div(1000);   // 0.5%
//         uint256 odds = calculateParlayOdds(p.odds);
//         uint256 risk = p.amount.mul(odds).div(100);
//         require( MAX_BET > risk, "Parlay payout too large");
//         p.odds = _odds;
