...
contract JuiceBookSwap {
    ...
    constructor( IERC20 _JCE, IWDAI _wdai, IUniswapV2Router02 _router ) {
        JCE = _JCE;
        wdai = _wdai;
        dai = wdai.wrappedToken();
        router = _router;
        factory = IUniswapV2Factory(_router.factory());
        pair = IUniswapV2Factory(_router.factory()).getPair(address(JCE), address(wdai));

        wdai.approve(address(_router),uint(-1));
